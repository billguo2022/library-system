...
#define SERVER_H_PORT 43947 // Replace with the actual port number for ServerH
...
void sendBookStatuses(const std::unordered_map<std::string, int>& bookData, int sockfd, const struct sockaddr_in& servaddr) {
    ...
    std::cout << "Server H sent book statuses to the Main Server.\n";
}

int main() {
    std::unordered_map<std::string, int> bookData = readBookData("input_files/history.txt"); 
    int sockfd;
    ...
    servaddr.sin_port = htons(SERVER_H_PORT);
    ...
    std::cout << "Server H is up and running using UDP on port " << SERVER_H_PORT << ".\n";
    ...
    while (true) {
        ...
        std::cout << "Server H received " << bookCode << " code from the Main Server.\n";
        ...
        std::cout << "Server H finished sending the status of code " << bookCode << " to the Main Server using UDP on port " << SERVER_H_PORT << ".\n";
    }
